...
export async function getReminders(token) {
  const response = await fetch(`${DATABASE_URL}/reminders.json?auth=${token}`);
...
export async function updateReminder(id, reminder, token) {
  const response = await fetch(`${DATABASE_URL}/reminders/${id}.json?auth=${token}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      ...reminder,
